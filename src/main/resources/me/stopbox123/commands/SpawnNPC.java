...
public class SpawnNPC implements CommandInterface {

	private static Main plugin = (Main) Bukkit.getPluginManager().getPlugin("BukkitNPC");
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
			...
			if (args[0].equalsIgnoreCase("create") && p.hasPermission("bukkitnpcs.*") && args.length == 2 || args[0].equalsIgnoreCase("create") && p.hasPermission("bukkitnpcs.create") && args.length == 2) {
				try {
					...
					//File file = new File(plugin.getDataFolder(), nmsEntity.displayName + ".yml");
					//FileConfiguration config = YamlConfiguration.loadConfiguration(file);
					//config.save(file);
					//config.set("displayName", nmsEntity.displayName);
					//config.set("loc.world", nmsEntity.world);
					//config.set("loc.X", nmsEntity.locX);
					//config.set("loc.Y", nmsEntity.locY);
					//config.set("loc.Z", nmsEntity.locZ);
			//		plugin.saveResource(nmsEntity.displayName + ".yml", false);
				} catch (Exception e) {
